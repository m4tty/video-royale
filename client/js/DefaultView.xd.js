...
dojo.declare("videoroyale.DefaultView", [dijit.layout.ContentPane, dijit._Templated, dijit._Container], {
	...
	destroy: function () {
		...
		// Note from Bob:
		// Just to save whomever works on implementing destroy a little legwork, a few notes:
		// (1) destroy, destroyRecursive, and destroyDescendants are all defined/implemented in
		//     dijit._WidgetBase. See http://dojotoolkit.org/api/1.6/dijit/_WidgetBase
		// (2) All implementations for destroy* in dijit._WidgetBase declare an exclusion for
		//     support of _Templated widgets. So...we might have some issues here.
		// (3) The boolean parameter on the destroy* methods is "preserveDom"; it specifies
		//     whether to leave the DOM structure alone (true) or tear down the affected DOM
		//     nodes (false) during destroy.
		// (4) Not sure why the workaround for IE; but since this code came from Fema's
		//     dv gradebook view, I would bet someone on that team could shed some light there.
		// (5) A link that might be handy: http://weierophinney.net/matthew/archives/208-Recursively-Destroying-Dojo-ContentPanes.html
		// (6) *** Don't forget to unsubscribe (or verify destroy takes care of this) from any
		//     named events (such as "/ecollege/showPeopleProfile") ***
		
        // destroy any currently loaded dijits
		if (dojo.isIE) { // workaround for IE
